...
export function ToastProvider({ children }: { children: React.ReactNode }) {
    ...
    const addToast = useCallback(({ title, description, type = "default", duration = 3000 }: Omit<Toast, "id">) => {
    ...
    return (
        ...
        >
            ...
            <div className="flex-1">
                {toast.title && <h4 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">{toast.title}</h4>}
                {toast.description && <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-1">{toast.description}</p>}
            </div>
