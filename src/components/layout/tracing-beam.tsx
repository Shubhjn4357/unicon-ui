    ...
    return (
      <div ref={ref} className={cn("relative w-full max-w-4xl mx-auto", className)} {...props}>
        <div className="absolute -left-4 top-0 bottom-0 w-px bg-border hidden md:block">
          <motion.div
            className="absolute top-0 -left-px w-[3px] h-[3px] rounded-full bg-primary origin-top"
            style={{ top: y }}
          >
            <div className="absolute top-0 left-[-6px] w-[15px] h-[15px] rounded-full bg-primary/20 animate-pulse" />
          </motion.div>
          <motion.div
        ...
        <div className="pl-4 md:pl-12">{children}</div>
      </div>
    )
  }
)
